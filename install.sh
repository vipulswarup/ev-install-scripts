...
cd ..
cp share_oob.zip /opt/alfresco-5.0.d/tomcat/webapps/share.war
cp alfresco_oob.zip /opt/alfresco-5.0.d/tomcat/webapps/alfresco.war
...
#to be done manually
cp evThemes_eisenVaultTheme.amp /opt/alfresco-5.0.d/amps_share/
cp share-amp.amp /opt/alfresco-5.0.d/amps_share/
cp repo-amp.amp /opt/alfresco-5.0.d/amps/
/opt/alfresco-5.0.d/bin/apply_amps.sh -force

cp ev_share_header_modules.jar /opt/alfresco-5.0.d/tomcat/shared/lib

#step 5 make a temp directory
mkdir /tmp/Alfresco

#step 6 - remove old alfresco, solr4 and share log files
rm /opt/alfresco-5.0.d/*.log
